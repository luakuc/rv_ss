...
uint64_t plic_claim(void)
{
    ...
    // 64bit ? 32bit?
    uint64_t *claim_address = (uint64_t*)SIFIVE_PLIC_S_CLAIM_COMPLETE + cpu_id * 0x2000;
...
void plic_complete(uint64_t irq)
{
    ...
    uint64_t *complete_address = (uint64_t*)SIFIVE_PLIC_S_CLAIM_COMPLETE + cpu_id * 0x2000;
